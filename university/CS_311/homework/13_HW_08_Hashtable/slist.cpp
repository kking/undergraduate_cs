...
// RETURN: the position (>=1) if a node is found, 0 otherwise
...
// PARAMETER element: the element whose key is to be found
// RETURN: the el_t object if found, otherwise a blank el_t
el_t slist::search2(el_t element) { 
	el_t blank; // a blank el_t object
	
	// key has only the ID part.
	// In the slist look for matching el_t in Elem
	Node *thisP = this->Front;

	while (thisP != NULL) {
		if (thisP->Elem.getkey() == element.getkey()) {
			return thisP->Elem; // return the el_t object if found
		}
		thisP = thisP->Next;
	}

	return blank; // return the blank el_t if not found
}
