...
// Each node has an Up link to make it easy to go up the tree. It must be set as nodes 
// are inserted. Some deletion cases require Up link to be updated. Couts are there to 
// test to make sure you set the Up links correctly!!!! (Fall 19)
// 
// HW#: 		HW4 BST
// NAME: 		King
// COMPILER: 		g++
...
// 
// INSTRUCTIONS: 	Look for ** comments to complete this file for HW4
...
void BST::INorderTraversal(Vertex *V)
{
	if (V != NULL)
	{
		...
		cout << (V->Elem) << endl; // ** display V's element, height and balance and do endl; 
...
void BST::DeleteVertex(el_t E)
{
	...
	{ 
		...
		if ((Root->Left != NULL) && (Root->Right == NULL)) // ** and it has only the left subtree
		{
			...
			Root->Up = NULL; // making sure the new root's UP is NULL
			delete oldRoot; // and the old root has been deleted (?)
			return;
		}
...
void BST::remove(Vertex *V, Vertex *P)
{
	...
	{ 
		...
		V->Up = P; // ** Make V's right child point UP to the parent;   
		cout << C->Elem << " points up to " << C->Up->Elem << endl;
		delete V; // ** Be sure to delete V
		// *** EC call from P to adjust height and BF 
	} //end of V with right child
	else if ((V->Left != NULL) && (V->Right != NULL)) // V has two children (case 3)
	{ 
...
// end of class implementation bintree.cpp
